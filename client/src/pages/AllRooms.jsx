  ...
  const matchesPriceRange = (room) => {
    return (
      ...
      selectedFilters.priceRange.some((range) => {
        ...
        return room?.pricePerNight >= min && room?.pricePerNight <= (max || Infinity);
      })
    );
  };
          ...
          {(filteredRooms ?? []).map((room) => (
            <div key={room?._id} className="w-full flex flex-col items-start">
              <div className="flex flex-col md:flex-row items-start gap-6 w-full">
                ...
                <div className="flex flex-col justify-between w-full">
                  <p className="text-sm text-gray-500">{room?.hotel?.city ?? 'City'}</p>
                  <h2
                    className="text-xl md:text-2xl font-playfair text-gray-800 cursor-pointer mt-1"
                    onClick={() => {
                      navigate(`/rooms/${room?._id}`);
                  ...
                  <p className="text-xl font-medium text-gray-700 mt-4">
                    {currency} {room?.pricePerNight ?? 0} <span className="mx-1">/</span>night
                  </p>
                </div>
              </div>
